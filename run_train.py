...
MAX_TRAIN_IMAGES = 200   # 100~500 권장, None이면 전체 사용
MAX_TEST_IMAGES  = 100   # None이면 전체 사용

MIN_FREQ = 3     # 이 횟수 미만으로 등장하는 라벨은 버림
TOP_K    = 200     # 빈도 상위 K개만 사용 (None이면 제한 없음)
...
DEBUG = True  # 점검 끝나면 False로

SAVE_COMPRESSED = False     # 디버깅/개발: False (빠름). 최종 산출물: True (용량 절약)
...
'''
def print_per_class_accuracy(y_true, y_pred, id_to_label, top_k_by_support=20, min_support=5):
    """테스트셋 클래스별 정확도 표 (상위 지원수 / 최악 정확 클래스)"""
    y_true = np.asarray(y_true); y_pred = np.asarray(y_pred)
    supp = Counter(y_true.tolist())
    correct = Counter(y_true[y_true == y_pred].tolist())

    rows = []
    for cid, n in supp.items():
        hit = correct.get(cid, 0)
        acc = hit / n if n > 0 else 0.0
        # id_to_label 키가 str일 수도 있음에 유의
        label = id_to_label.get(cid) or id_to_label.get(str(cid), str(cid))
        rows.append((cid, label, n, hit, acc))

    # 1) 지원수(표본 수) 상위 클래스들
    rows_by_support = sorted(rows, key=lambda r: r[2], reverse=True)
    print(f"\n# Per-class accuracy on TEST (top {top_k_by_support} by support)")
    print("| rank | class_id | label | support | correct | acc |")
    print("| ---: | -------: | :---- | ------: | ------: | --: |")
    for i, (cid, label, n, hit, acc) in enumerate(rows_by_support[:top_k_by_support], 1):
        print(f"| {i} | {cid} | {_md_escape(label)} | {n} | {hit} | {acc:.3f} |")

    # 2) 최악 정확 클래스(의미있는 지원수만)
    worst = [r for r in rows if r[2] >= min_support]
    worst.sort(key=lambda r: r[4])  # acc 오름차순
    if worst:
        print(f"\n# Worst classes on TEST (min_support ≥ {min_support})")
        print("| rank | class_id | label | support | correct | acc |")
        print("| ---: | -------: | :---- | ------: | ------: | --: |")
        for i, (cid, label, n, hit, acc) in enumerate(worst[:min(10, len(worst))], 1):
            print(f"| {i} | {cid} | {_md_escape(label)} | {n} | {hit} | {acc:.3f} |")
'''
...
def build_npz(rows, text_id, src_to_img):
    ...
    for r in rows:
        ...
        if cid is None: 
            unk += 1
            continue  # test에서 미지 클래스는 제외 (표본을 버림)
...
def main():
    ...
    # === A안: 이미지 페어 자체 랜덤 샘플링 ===
    # (재현성: 위에서 random.seed(SEED) 이미 호출되어 있으니 그대로 random.sample 사용)
    ...
    #print("[Top-5]", [f"{t} x{c}" for t, c in text_freqs[:5]])

    #파일로 저장
    #with (OUT_ROOT/"train"/"vocab_train.json").open("w", encoding="utf-8") as f:
    #    json.dump(text_id, f, ensure_ascii=False, indent=2)
    # (선택) 병합 CSV 저장
    ...
    #    for r in train_bbox:
    #        w.writerow([r["src"], r["idx"], r["text_norm"], text_id[r["text_norm"]],
    #                    r["x1"], r["y1"], r["x2"], r["y2"]])
    ...
    #if DEBUG:
    #    debug_oov("train", len(ytr), unk_tr)
    #    debug_dataset("train", Xtr, ytr, text_id)
    ...
    if test_pairs:
        ...
        if Xte is not None:
            #if DEBUG:
            #    debug_oov("test", len(yte), unk_te)
            #    debug_dataset("test", Xte, yte, text_id)
            # NPZ 저장부
            if SAVE_COMPRESSED:
    ...
    trainer = Trainer(
        ...
        evaluate_sample_num_per_epoch=None #1000  
    )
    trainer.train()
